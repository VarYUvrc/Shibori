...
- 接続中モニターのチェックでメインを選択します。メインは常に1台だけです。
- メイン変更時は選択した画面の座標を基準に全画面を平行移動し、メイン停止時は残存画面を原点基準に再配置します。
- 表示構成は`SDC_VALIDATE`で検証してから適用し、path数、mode数、mode index、画面座標を通常ログへ記録します。
- CCDはvirtual modeとvirtual refresh rateの両方を認識して読み書きし、virtual-aware pathでは分割されたmode indexを使用します。
...
再起動後にモニターが見つからない場合は、Shiboriを起動してInfo内の復旧手順を確認し、Windowsの表示設定で「検出」を行ってから復旧します。スタートアップ用ショートカットの末尾に `--startup` を付けると、更新モーダルを表示せず起動できます。通常起動では安定版・プレリリースを含む新しいGitHub Releaseを通知します。現状はタスクトレイ常駐ではなく通常ウィンドウです。
